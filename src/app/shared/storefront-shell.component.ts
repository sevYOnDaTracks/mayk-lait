...
@Component({ selector: 'app-storefront-shell', imports: [RouterOutlet, RouterLink, RouterLinkActive, CurrencyPipe, OnboardingComponent], template: `
  <header class="site-header">
    ...
    <div class="header-actions">
      @if(auth.isAdmin()){<a routerLink="/admin" class="mode-switch desktop-mode-switch">Mode administrateur</a>}
      <a [routerLink]="auth.user() ? '/compte' : '/connexion'" class="account-trigger" [attr.aria-label]="auth.user() ? 'Mon compte' : 'Se connecter'" [attr.title]="auth.user() ? 'Mon compte' : 'Se connecter'">
        @if (auth.user(); as user) {
          @if (user.photoURL) { <img [src]="user.photoURL" alt=""> }
          @else { <span>{{ initials() }}</span> }
        } @else {
          <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="4"/><path d="M4 21a8 8 0 0 1 16 0"/></svg>
        }
      </a>
      <button class="cart-trigger icon-cart" (click)="cart.open.set(true)" aria-label="Ouvrir le panier" title="Panier"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 4h2l2.2 10.2a2 2 0 0 0 2 1.6h7.7a2 2 0 0 0 2-1.6L20 8H6"/><circle cx="10" cy="20" r="1"/><circle cx="17" cy="20" r="1"/></svg><span>{{ cart.count() }}</span></button>
      <button class="menu-trigger" (click)="menuOpen.set(!menuOpen())" [attr.aria-expanded]="menuOpen()" aria-label="Ouvrir le menu"><i></i><i></i></button>
    </div>
    @if (menuOpen()) {
      <nav class="mobile-menu">
        <a routerLink="/" (click)="menuOpen.set(false)">Accueil</a>
        <a routerLink="/boutique" (click)="menuOpen.set(false)">Boutique</a>
        <a [routerLink]="auth.user() ? '/compte' : '/connexion'" (click)="menuOpen.set(false)">{{ auth.user() ? 'Mon compte' : 'Se connecter' }}</a>
        @if(auth.isAdmin()){<a routerLink="/admin" class="mobile-admin-link" (click)="menuOpen.set(false)"><span>ADMIN</span> Mode administrateur <b>→</b></a>}
      </nav>
    }
  </header>
  ...
  @if (cart.open()) { <div class="drawer-backdrop" (click)="cart.open.set(false)"></div><aside class="cart-drawer" aria-label="Panier"><div class="drawer-head"><h2 aria-label="Ton panier"><svg class="drawer-cart-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M6 8h12l1 13H5L6 8Z"/><path d="M9 9V6a3 3 0 0 1 6 0v3"/></svg></h2><button class="icon-button" (click)="cart.open.set(false)" aria-label="Fermer">×</button></div>
...
export class StorefrontShellComponent {
  ...
  initials() {
    ...
    return `${user?.firstName?.charAt(0) ?? ''}${user?.lastName?.charAt(0) ?? ''}`.toUpperCase() || 'MK';
  }
}
